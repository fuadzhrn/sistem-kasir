    ...
    <div class="card__body form-grid">
        @if (auth()->user()->isOwner())
            <div class="form-group">
                ...
                <select class="form-select @error('branch_id') is-error @enderror" id="receipt-branch" name="branch_id" required data-receipt-branch>
        ...
        <div class="form-group">
            ...
            <input class="form-control @error('receipt_date') is-error @enderror" id="receipt-date" name="receipt_date" type="date" value="{{ old('receipt_date', now()->toDateString()) }}" max="{{ now()->toDateString() }}" required data-receipt-date>
            @error('receipt_date')<span class="form-error">{{ $message }}</span>@enderror
        </div>
        <div class="form-group">
            <label class="form-label" for="receipt-supplier">Supplier</label>
            <input class="form-control @error('supplier_name') is-error @enderror" id="receipt-supplier" name="supplier_name" type="text" value="{{ old('supplier_name') }}" maxlength="255" placeholder="Opsional" data-receipt-supplier>
            @error('supplier_name')<span class="form-error">{{ $message }}</span>@enderror
        </div>
        <div class="form-group">
            <label class="form-label" for="receipt-notes">Catatan</label>
            <textarea class="form-textarea @error('notes') is-error @enderror" id="receipt-notes" name="notes" maxlength="1000" placeholder="Opsional" data-receipt-notes>{{ old('notes') }}</textarea>
